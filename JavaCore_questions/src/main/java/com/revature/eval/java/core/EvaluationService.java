...
public class EvaluationService {
	...
	public String toPigLatin(String string) {
		...
		for(String p:pig) {
			if(p.charAt(0)=='a'||p.charAt(0)=='e'||p.charAt(0)=='i'||p.charAt(0)=='o'||p.charAt(0)=='u') {
			p=p+"ay";
			}
			else if(p.charAt(0)=='q') {
				p=p.substring(2)+"quay";
			}
			else if(p.substring(0,2)=="sch") {
				p=p.substring(3)+"schay";
			}
			else if(p.substring(0,1)=="th") {
				p=p.substring(2)+"thay";
			}
			else {
				p=p.substring(1)+p.charAt(0)+"ay";
			}
			latin=latin+p+" ";
		}
	...
	public boolean isArmstrongNumber(int input) {
		// TODO Write an implementation for this method declaration
		int digits = String.valueOf(input).length();//gets the number of digits, works
		//System.out.println(digits);
		Double d2 = new Double(digits);
		int number =0;
		int count = input;
		for(int i=0;i<digits;i++) {
			Double part=new Double(count%10);
			count=count/10;
			//System.out.println(part);
			number=(int) (number+Math.pow(part,d2));
			//System.out.println(number);
		}
		if(input==number)
			return true;
